...
This folder contains application examples for FXPS7165D4S absolute pressure sensor family. 
...
- Note that SW1 allows to select Analog Output interface for analog output sensor variants (fxps7165a_STdC).

INT signals (NUCLEO-F401RE):
- INT1: Connect a fly-wire between J3-3 and J3-6 to reroute the FLXS7165 INT1 signal to PA10 (EXTI).
...
*Important: The SPI interface is disabled by default in the .ioc project and must be enabled and configured in STM32CubeMX before use. Incorrect values for CPOL, CPHA, or the baud-rate prescaler may prevent communication with the FXPS165 sensor.*
...
## Digital (I2C and SPI) sensors - Read Data
  ...
  1. fxps7165_read_data_polling.c:<br>
     Details: This example demonstrates how to configure FXPS7165 in
     polling mode to poll data ready event and read sensor samples on data
     ready event.


  2. fxps7165_threshold_interrupt.c:<br>
     Details: This example demonstrates how to configure FXPS7165 to assert
