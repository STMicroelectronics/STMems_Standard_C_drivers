...
This folder contains application examples for FXPS7550D4S absolute pressure sensor family. 
...
- Note that SW1 allows to select Analog Output interface for analog output sensor variants (fxps7550a_STdC).

INT signals (NUCLEO-F401RE):
- INT1: Connect a fly-wire between J3-3 and J3-6 to reroute the FLXS7550 INT1 signal to PA10 (EXTI).
...
*Important: The SPI interface is disabled by default in the .ioc project and must be enabled and configured in STM32CubeMX before use. Incorrect values for CPOL, CPHA, or the baud-rate prescaler may prevent communication with the FXPS7550 sensor.*
...
## Digital (I2C and SPI) sensors - Read Data
  ...
  1. fxps7550_read_data_polling.c:<br>
     Details: This example demonstrates how to configure FXPS7550 in
     polling mode to poll data ready event and read sensor samples on data
     ready event.


  2. fxps7550_threshold_interrupt.c:<br>
     Details: This example demonstrates how to configure FXPS7550 to assert
